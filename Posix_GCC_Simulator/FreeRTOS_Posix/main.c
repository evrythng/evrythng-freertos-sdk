...
static unsigned portLONG uxCheckTaskHookCallCount = 0;
static unsigned portLONG uxQueueSendPassedCount = 0;
/*-----------------------------------------------------------*/

int main( void )
{
xTaskHandle hUDPTask, hMQTask;
xQueueHandle xUDPReceiveQueue = NULL, xIPCQueue = NULL;
...
struct sockaddr_in xReceiveAddress;
	...
	xTaskCreate( prvUDPTask, "UDP RxTx", configMINIMAL_STACK_SIZE, xUDPReceiveQueue, tskIDLE_PRIORITY + 1, &hUDPTask );
...
xQueueHandle xUDPReceiveQueue = (xQueueHandle)pvParameters;
	...
	if ( iSocketSend != 0 )
	{
		...
		iReturn = !inet_aton( mainUDP_SEND_ADDRESS, &( xSendAddress.sin_addr.s_addr ) );
		...
		for (;;)
		{
			if ( pdPASS == xQueueReceive( xUDPReceiveQueue, &xPacket, 5000 ) )
			{
...
xMessageObject xTxMsg, xRxMsg = { { 0 } };
	...
	for ( ;; )
	{
		...
		{
			if ( pdTRUE != xQueueReceive( (xQueueHandle)pvParameters, &xRxMsg, 5000 ) )
			{
...
static unsigned portLONG uxLastHookCallCount = 0, uxLastQueueSendCount = 0;
